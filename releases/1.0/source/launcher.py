...
M = {
    ...
    "text_md":     "#AAAAAA",
    "text_lo":     "#666666",
    "ok":          "#FFFFFF",   # monochrome — success state = bright white
    "danger":      "#FFFFFF",   # error: word "ERROR" carries meaning, not color
    "warn":        "#FFFFFF",
}
...
class App:
    ...
    def _build_ui(self):
        ...
        # Single horizontal bar: dark background with a white "fill" underlay
        # that grows with progress, status text overlaid via place().
        self.status_bar = ctk.CTkFrame(
            content, fg_color=M["surface_2"], height=36, corner_radius=4,
        )
        self.status_bar.pack(fill="x", pady=(14, 6))
        self.status_bar.pack_propagate(False)

        # Fill (grows left→right with progress). White-on-dark.
        self.status_fill = ctk.CTkFrame(
            self.status_bar, fg_color=M["primary"], corner_radius=4,
        )
        self.status_fill.place(x=0, y=0, relwidth=0.0, relheight=1.0)

        # Status text. Use a mid-gray that's readable on both white-fill and dark-bg.
        ...
        self.status_label.place(x=14, y=0, relwidth=1.0, relheight=1.0)
    ...
    def _set_status(self, msg, color=None):
        ...
        # `color` kept for back-compat with callers; text color stays mid-gray
        # so it's readable on both dark bg and white fill.

    def _set_progress(self, frac: float):
        """0.0-1.0 — drives the white fill width inside the status bar."""
        frac = max(0.0, min(1.0, frac))
        try: self.status_fill.place_configure(relwidth=frac)
        except Exception: pass
    ...
    def _handle_done(self, result: dict):
        ...
        if not result.get("ok"):
            ...
            self._set_status("Failed", M["danger"])
            self._set_buttons_enabled(True)
            return
        ...
        if "recipeSummary" in result:
            ...
            self._set_status("Analyze complete", M["ok"])
            ...
            self._set_status(
                f"Done · {r.get('ok')} ok, {r.get('fail')} failed · {r.get('elapsed', 0):.1f}s",
                M["ok"],
            )
        ...
        self._set_buttons_enabled(True)
        # Reset progress for analyze (which doesn't use it)
        if "recipeSummary" in result:
            self._reset_progress()
