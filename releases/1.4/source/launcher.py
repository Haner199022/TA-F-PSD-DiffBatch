...
def save_appearance_mode(mode: str) -> None:
    """Persist Dark / Light / System to the user's preset file."""
...
# ---------------------------------------------------------------------------
# Theme — TA-F monochrome (Dark + Light)
# ---------------------------------------------------------------------------

# Load saved appearance mode early — must run before CTk widgets are built.
# Falls back to "dark" when preset file is missing/has no key (preserves
# pre-v1.4 launcher behavior).
def _load_saved_appearance_mode() -> str:
    try:
        if PRESETS_PATH.exists():
            data = json.loads(PRESETS_PATH.read_text(encoding="utf-8"))
            mode = (data or {}).get("appearance_mode")
...
M_LIGHT = {
    ...
    "primary_dk":  "#333333",   # hover (slightly lighter black)
    "on_primary":  "#FFFFFF",   # text on black button
    "secondary":   "#444444",   # in-progress accent (darker gray)
    "text_hi":     "#000000",
    "text_md":     "#555555",
    "text_lo":     "#999999",
...
def C(key: str) -> tuple:
    ...
    by appearance mode. Drop-in replacement for the old M[key] style."""
...
# NOTE: there is no `M` dict anymore. All call sites use `C("key")` which
# returns the (light, dark) tuple CTk needs for live mode switching. For the
# few non-CTk widgets (tk.PanedWindow, etc.) that don't accept tuples, read
# `_current_palette()[key]` and reconfigure on theme change.
...
class App:
    ...
    def _build_ui(self):
        ...
        self.theme_var = ctk.StringVar(value=_load_saved_appearance_mode())
    ...
    def _on_theme_change(self, mode: str):
        ...
        try:
            ...
            print(f"[theme] PanedWindow reconfigure failed: {exc}", file=sys.stderr)
